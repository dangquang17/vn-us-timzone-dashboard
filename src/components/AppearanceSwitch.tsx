...
function AppearanceSwitchComponent({
  mode,
  resolvedTheme,
  onChange,
}: AppearanceSwitchProps) {
  const resolvedLabel = resolvedTheme === "dark" ? "Dark" : "Light";
  const modeLabel = mode === "system" ? "System" : mode[0].toUpperCase() + mode.slice(1);
...
export const AppearanceSwitch = memo(AppearanceSwitchComponent);
